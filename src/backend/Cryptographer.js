...
export function generateUniqueSalt() {
    let prospectiveSalt = makeSalt();
    while (Database.saltIsFound(prospectiveSalt)) {
        prospectiveSalt = makeSalt();
    }
    return prospectiveSalt;
}

function makeSalt(length=512) {
 ...
 * @returns {boolean}
 */
export function slowEquals(str1, str2) {
